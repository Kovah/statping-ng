<template>
  <div>
    ...
    <div v-else>
      <table class="table">
        ...
        <tr>
          ...
          <th scope="col" class="d-none d-md-table-cell" style="width: 130px">
            ...
            <div class="btn-group float-right" role="group">
              <a @click="list_timeframe='3h'" type="button" class="small"
                 :class="{'text-success': list_timeframe==='3h', 'text-muted': list_timeframe!=='3h'}">3h</a>
              <a @click="list_timeframe='12h'" type="button" class="small"
                 :class="{'text-success': list_timeframe==='12h', 'text-muted': list_timeframe!=='12h'}">12h</a>
              <a @click="list_timeframe='24h'" type="button" class="small"
                 :class="{'text-success': list_timeframe==='24h', 'text-muted': list_timeframe!=='24h'}">24h</a>
              <a @click="list_timeframe='7d'" type="button" class="small"
                 :class="{'text-success': list_timeframe==='7d', 'text-muted': list_timeframe!=='7d'}">7d</a>
            </div>
          </th>
          <th scope="col"></th>
        </tr>
        ...
        <draggable id="services_list" tag="tbody" v-model="servicesList" handle=".drag_icon">
          <tr v-for="(service, index) in servicesList" :key="service.id">
            ...
            <td class="text-right">
              <div class="btn-group">
                <button :disabled="loading" v-if="$store.state.admin"
                ...
                <button :disabled="loading" v-if="$store.state.admin" @click.prevent="deleteService(service)"
      ...
      <div>
        <button :disabled="loading" @click.prevent="gotoPage(1)"
                class="btn btn-sm btn-outline-secondary">1</button>
        <button :disabled="loading" @click.prevent="gotoPage(2)"
                class="btn btn-sm btn-outline-secondary">2</button>
        <button :disabled="loading" @click.prevent="gotoPage(3)"
...
export default {
  ...
  methods: {
    ...
    async gotoPage(page) {
      const services = await Api.servicesForPage(page)
    ...
    async update() {
      const services = await Api.servicesForPage(1)
      this.$store.commit('setServices', services)
    }
  }
}
